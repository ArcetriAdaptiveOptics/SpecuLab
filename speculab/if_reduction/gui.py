...
from pipeline_lib import classify_function, load_custom_function, load_predefined_functions, run_pipeline
...
class StepWidget(QWidget):
    def __init__(self, action, action_list):
        super().__init__()
        self.action = action
        ...
        self.func_selector.currentTextChanged.connect(self.on_function_changed)
...
#        param_widgets = self.generate_widgets_for_params(action)
#        for widget in param_widgets:
#            param_layout.addWidget(widget)
        self.layout.addLayout(self.params_layout )


        # Show/hide depending on initial selection
        self.user_func_input.setVisible(self.func_selector.currentText() == "Custom")
        ...
        self.on_function_changed(self.func_selector.currentText())
    ...
    def load_custom_file(self):
        ...
        for name in dir(module):
            ...
            if callable(attr):
                ...
                self.func_selector.addItem(name)
...
class PipelineGUI(QWidget):
    ...
    def refresh_list(self):
        ...
        for idx, action in enumerate(self.action_list.actions):
            ...
            widget = StepWidget(action, self.action_list)
    ...
    def get_func_and_param_lists(self):
        ...
        for action in self.action_list.actions:
            if not action.active:
                continue
            step_widget = self.get_step_widget(action.step_number - 1)
            params = step_widget.get_parameters().values()
            func_name = step_widget.action.func_name
            func_list.append(self.action_list.functions[func_name])
